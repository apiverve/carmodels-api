...
class CarmodelsValidationException implements Exception {
  final List<String> errors;

  CarmodelsValidationException(this.errors);

  @override
  String toString() => 'CarmodelsValidationException: ${errors.join("; ")}';
}
...
/// Car Models API client.
///
/// For more information, visit: https://apiverve.com/marketplace/carmodels?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [make] (required) - The make (manufacturer) of the car (e.g., Toyota, Honda, Ford)
/// * [year] - Filter by year (1984-present) [min: 1984, max: 2030]
class CarmodelsClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'make': ValidationRule(type: 'string', required: true),
    'year': ValidationRule(type: 'integer', required: false, min: 1984, max: 2030),
  };

  CarmodelsClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/carmodels',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CarmodelsValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CarmodelsValidationException(errors);
    }
  }

  /// Execute a request to the Car Models API.
  ///
  /// Parameters are validated before sending the request.
  Future<CarmodelsResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CarmodelsException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CarmodelsResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CarmodelsException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CarmodelsException('Resource not found');
      } else {
        throw CarmodelsException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CarmodelsException) rethrow;
      throw CarmodelsException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Car Models API client.
class CarmodelsException implements Exception {
  final String message;

  CarmodelsException(this.message);

  @override
  String toString() => 'CarmodelsException: $message';
}
