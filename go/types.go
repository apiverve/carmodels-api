// Package carmodels provides a Go client for the Car Models API.
//
// For more information, visit: https://apiverve.com/marketplace/carmodels?utm_source=go&utm_medium=readme
package carmodels
...
// Request contains the parameters for the Car Models API.
//
// Parameters:
//   - make (required): string - The make (manufacturer) of the car (e.g., Toyota, Honda, Ford)
//   - year: integer - Filter by year (1984-present) [min: 1984, max: 2030]
type Request struct {
	Make string `json:"make"` // Required
...
func (r *Request) Validate() error {
	rules := map[string]ValidationRule{
		...
		"year": {Type: "integer", Required: false, Min: float64Ptr(1984), Max: float64Ptr(2030)},
	}
...
// ResponseData contains the data returned by the Car Models API.
type ResponseData struct {
	Count int `json:"count"`
	FilteredOn []string `json:"filteredOn"`
	Cars []CarsItem `json:"cars"`
}

// CarsItem represents an item in the Cars array.
type CarsItem struct {
	Make string `json:"Make"`
	CityMPG string `json:"CityMPG"`
	CityELEC string `json:"CityELEC"`
	CombMPG string `json:"CombMPG"`
	CombELEC string `json:"CombELEC"`
	Cyl string `json:"Cyl"`
	Displace string `json:"Displace"`
	Drive string `json:"Drive"`
	Fuel string `json:"Fuel"`
	HighwELEC string `json:"HighwELEC"`
	HighwMPG string `json:"HighwMPG"`
