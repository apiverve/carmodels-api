...
class CarmodelsAPIClientError(Exception):
...
class ValidationError(CarmodelsAPIClientError):
...
class CarmodelsAPIClient:
    ...
    VALIDATION_RULES = {"make": {"type": "string", "required": True}, "year": {"type": "integer", "required": False, "min": 1984, "max": 2030}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the CarmodelsAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises CarmodelsAPIClientError: If API key is invalid
        """
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[CarmodelsAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns CarModelsData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises CarmodelsAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise CarmodelsAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise CarmodelsAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise CarmodelsAPIClientError(f"Request failed: {str(e)}")
